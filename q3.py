...
df_avg_rating = df.groupBy("City").agg(avg("Rating")).withColumnRenamed("SUM(Rating)", "AverageRating")
df_best = df_avg_rating.sort(desc("AverageRating")).withColumn("RatingGroup", "Top")
df_worst = df_avg_rating.sort(asc("AverageRating")).withColumn("RatingGroup", "Bottom")
